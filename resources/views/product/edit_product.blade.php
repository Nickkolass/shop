  ...
  <div class="container-fluid">
    ...
    <div class="row">
      <form action="{{ route('product.update_product', $product->id) }}" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group">
          <input type="text" name="descriprion" value="{{ $product->descriprion }}" class="form-control" placeholder="Описание">
        </div>
        <div class="form-group">
          ...
          <div class="card-body card-block">
            <div class="row form-group">
              <div class="control-group" id="exampleInputFile">
                ...
                <td>
                  <img src="{{ asset($product->preview_image) }}" width='70' height='70' class="img img-responsive">
                </td>
              </div>
            </div>
          </div>
          ...
          <div class="card-body card-block">
            <div class="row form-group">
              <div class="control-group" id="fields">
                ...
                <div class="controls">
                  <div class="entry input-group upload-input-group">
                    <input class="form-control" name="product_images[]" type="file" value = "{{ asset($productImages) }}" multiple>
                  </div>
                  <label class="control-label" for="fields">Выбранные</label>
                </div>
                @foreach($productImages as $productImage)
                <td>
                  <img src="{{ asset('/storage/'.$productImage->file_path) }}" width='70' height='70' class="img img-responsive">
                </td>
                @endforeach
              </div>
            </div>
        ...
        <div class="form-group">
          <select name="category_id" class="form-control select2" style="width: 100%;">
            ...
            <option {{$category->id==$product->category_id ? 'selected' : ''}} value="{{$category->id}}"> {{$category->title}}
            </option>
            @endforeach
            </option>
          </select>
        ...
        <div class="form-group">
          <select name="group_id" class="form-control select2" style="width: 100%;">
            ...
            <option {{$group->id==$product->group_id ? 'selected' : ''}} value="{{$group->id}}"> {{$group->title}}
            </option>
            @endforeach
            </option>
          </select>
        ...
        <div class="form-group">
          <select name="tags[]" class="tags" multiple="multiple" data-placeholder="Выберите тег" style="width: 100%;">
            @foreach($tags as $tag)
            <option @foreach($productTags as $productTag) {{ $productTag->id == $tag->id ? 'selected' : '' }} value="{{$tag->id}}" @endforeach>
              {{$tag->title}}
            </option>
            @endforeach
          </select>
        ...
        <div class="form-group">
          <select name="colors[]" class="colors" multiple="multiple" data-placeholder="Выберите цвет" style="width: 100%;">
            ...
            <option @foreach($colorProducts as $colorProduct) {{ $colorProduct->id==$color->id ? 'selected' : '' }} value="{{$color->id}}" @endforeach>
              {{$color->title}}
            </option>
            @endforeach
          </select>

  ...
  <form action="{{ route('api.filter_api', $data['category']['title']) }}" method="post" enctype="multipart/form-data">
    ...
    <div class="cd-filter-block">
      ...
      <ul class="cd-filter-content cd-filters list">
        <li>
          ...
          <input class="filter" style="width:125px; padding: 5px;" name="filter[prices][{{$key}}]" type="number" min="{{$data['filterable']['prices']['min']}}" max="{{$data['filterable']['prices']['max']}}" value="{{ !empty($data['filter']['prices'][$key]) ? $data['filter']['prices'][$key] : '' }}" placeholder="{{$data['filterable']['prices'][$key]}}">
          @endforeach
        </li>
      </ul> 
    </div>
    ...
    <div class="cd-filter-block">
      ...
      <ul class="cd-filter-content cd-filters list">
        ...
        <li>
          <input class="filter" type="radio" name="paginate[orderBy]" value='ASC' @checked($data['paginate']['orderBy'] == 'ASC')>
      ...
      <div class="cd-filter-block">
        ...
        <ul class="cd-filter-content cd-filters list">
          @foreach ($optionValues as $optionValue)
            ...
            <input class="filter" name="filter[optionValues][{{$optionValue['option_id']}}][]" type="checkbox" value="{{$optionValue['id']}}" @checked(in_array($optionValue['id'], $data['filter']['optionValues'][$optionValue['option_id']]))>
            @else
            <input class="filter" name="filter[optionValues][{{$optionValue['option_id']}}][]" type="checkbox" value="{{$optionValue['id']}}">
            @endif
      ...
      <div class="cd-filter-block">
        ...
        <ul class="cd-filter-content cd-filters list">
          @foreach ($propertyValues as $propertyValue)
            ...
            <input class="filter" name="filter[propertyValues][{{$propertyValue['property_id']}}][]" type="checkbox" value="{{$propertyValue['id']}}" @checked(in_array($propertyValue['id'], $data['filter']['propertyValues'][$propertyValue['property_id']]))>
            @else
            <input class="filter" name="filter[propertyValues][{{$propertyValue['property_id']}}][]" type="checkbox" value="{{$propertyValue['id']}}">
            @endif
        ...
        @foreach ($data['filterable']['tags'] as $tag)
          <li>
            @if (!empty($data['filter']['tags']))
                <input class="filter" name="filter[tags][]" type="checkbox" value="{{$tag['id']}}" @checked(in_array($tag['id'], $data['filter']['tags']))>
            @else
        ...
        @foreach ($data['filterable']['salers'] as $saler)
          <li>
            @if (!empty($data['filter']['salers']))
              <input class="filter" name="filter[salers][]" type="checkbox" value="{{$saler['id']}}" @checked(in_array($saler['id'], $data['filter']['salers']))>
            @else

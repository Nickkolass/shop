...
@section('content')
    ...
    <section class="content">
        <div class="container-fluid">
            <form action="{{ route('admin.products.editProperties', $product->id) }}" method="post"
                            ...
                            <div class="col" style="padding: 5px;">
                                ...
                                <select name="category_id" id="category_id" class="categories" style="width: 100%;"
                                    ...
                                    @foreach($data['categories'] as $category_id => $category_title_rus)
                                        <option
                                            value="{{ $category_id }}" @selected($product->category_id == $category_id)>{{ $category_title_rus }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="w-100"></div>
                            ...
                            <div class="col" style="padding: 5px;">
                                ...
                                <select name="tags[]" class="tags" multiple="multiple" style="width: 100%;" required>
                                    @foreach($data['tags'] as $tag_id => $tag_title)
                                        <option
                                            value="{{ $tag_id }}" @selected($product->tags->contains($tag_id))>{{ $tag_title }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="w-100"></div>
